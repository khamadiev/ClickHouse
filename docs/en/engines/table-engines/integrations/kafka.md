...
When the `MATERIALIZED VIEW` joins the engine, it starts collecting data in the backround. This allows you to continually receive messages from Kafka and convert them to the required format using `SELECT`.
